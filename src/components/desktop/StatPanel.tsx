 ...
 *
 * 레이아웃은 레퍼런스를 따른다: `글리프 + 한국어 라벨` 왼쪽, **큰 우측 정렬 숫자**가 오른쪽.
 * 게이지는 줄 안에 끼워 넣지 않고 그 아래 전폭 1px 금선으로 깐다 —
      ...
      {max !== undefined && (
        <span className="stat-bar">
          {/* width 대신 scaleX로 채운다 — width 전환은 매 프레임 레이아웃을 다시 계산시킨다.
              색은 CSS의 --hud-gold 하나뿐이라 인라인 style은 채움 비율만 나른다
 ...
 * **테두리도 배경도 없다** — 레퍼런스에는 행을 감싸는 상자가 없고, 상자를 두르면
 * "칸막이 서랍" 인상이 생겨 촌스러워진다. 구분은 간격이 한다.
 */
function GrowthCell({ statKey, value }: { statKey: GrowthStatKey; value: number }) {
  ...
  return (
    <div className="stat-cell" title={`${STAT_NAMES[statKey]} ${value}`}>
      <AppIcon name={hudIcon} size={13} className="stat-icon" />
...
export function StatPanel() {
  ...
  return (
    ...
    >
      ...
      <HudSection label="자원" en="Resources" />
      ...
      <HudSection label="능력치" en="Attributes" />
      ...
      {/* 3구역: 생계 경고. 예전에는 구분선 아래 떠 있는 주석이라 "덧붙인 쪽지"로 보였다 —
          다른 구역과 같은 이중 언어 라벨을 주어 패널의 일부로 편입시킨다. */}
      <HudSection label="생계" en="Upkeep" />
