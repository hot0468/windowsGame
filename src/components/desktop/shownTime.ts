 ...
 * 미루는 조건이 `Daybreak`와 같은 하나이므로 **바탕화면 풍경·날짜칸·알림 팝업이 한꺼번에**
 * 넘어간다(설계자 지시: "행동 하자마자 시간대 바뀌지 말고 결과창에서 확인 누르거나
 * 시간대 관련 팝업이 뜨면 바뀌게").
 *
 ...
 */
export function useShownTime(): { day?: number; slot?: Slot; lagging: boolean } {
  const day = useGameStore((s) => s.state?.day)
  const slot = useGameStore((s) => s.state?.slot)
  const held = useResultOpen()
  const [shown, setShown] = useState<{ day?: number; slot?: Slot }>({ day, slot })

  useEffect(() => {
    if (held) return
    /* 같은 값이면 새 객체를 만들지 않는다 — 참조가 흔들리면 이걸 읽는 화면이 매 렌더 다시 그린다. */
    setShown((prev) => (prev.day === day && prev.slot === slot ? prev : { day, slot }))
  }, [day, slot, held])

  /* ⚠️ **첫 값은 미루지 않는다.** 판이 열리기 전 렌더에서는 미룰 '전 값'이 없어서,
     그대로 두면 잠금화면을 지나 판이 서는 한 프레임 동안 날짜가 비고 오후로 읽힌다. */
  const settled = shown.day === undefined ? { day, slot } : shown
  return { ...settled, lagging: settled.day !== day || settled.slot !== slot }
}
