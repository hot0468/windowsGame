 ...
 *
 * ## 규칙
 * - 판정은 **상한 대비 비율의, 상위 12종 평균**이다(2026-08-17 — 처음엔 15종 전부였다).
 *   절대값 합으로 재면 상한 999짜리 열두 개가 상한 100짜리 셋(평판·도덕·예의범절)을
 *   덮어 버려 그 셋을 올릴 이유가 사라진다.
 * - **하위 3종은 세지 않는다.** 15종 전부를 채워야 오르면 "이번 판은 화가로 키운다"류의
 *   페르소나 플레이가 덤프 스탯 몇 개 때문에 영원히 벌을 받고, 정답이 항상 균등
 *   그라인딩이 된다 — 숙련 보너스(`masteryBonusFor`)가 특화를 보상하는 것과 정면으로
 *   부딪히던 자리다. 셋을 눈감으면 두 시스템이 같은 방향을 가리킨다.
 *   ⚠️ **면제는 3종까지다**: `1 / LIFE_STAT_COUNT`가 C 문턱(10%) 아래여야 한 우물
 *   하나로 "두루의 보상"이 열리지 않는다(`lifeRank.test.ts`가 관계식으로 지킨다).
 *   ⚠️ 고루 올린 판에서는 하위 셋도 같은 높이라 값이 거의 안 변한다 — 도달 일수
 *   추정(C 90일·B 266일)은 그대로 유효하다.
...
/** 생활 등급이 세는 스탯 수 — 성장 스탯 15종 중 **상위 12종**(위 규칙 참고). */
export const LIFE_STAT_COUNT = 12
...
/**
 * 성장 스탯의 **상한 대비 비율, 상위 12종 평균**(0~1). 하위 3종은 면제다(위 규칙).
 *
...
export function lifeRatio(stats: Stats): number {
  const ratios = GROWTH_STAT_KEYS.map((key) => stats[key] / growthCap(key)).sort((a, b) => b - a)
  const sum = ratios.slice(0, LIFE_STAT_COUNT).reduce((acc, r) => acc + r, 0)
  const ratio = sum / LIFE_STAT_COUNT
  /* ⚠️ **문턱에 정확히 걸친 값을 문턱 아래로 떨어뜨리지 않는다.** 나눗셈 열두 번을
     더한 값이라 상한의 딱 95%를 채워도 0.9499999…가 나오고, 그러면 화면이
 ...
 * 최대 `SS+5`가 되고 그 위는 없다. **그 지점은 상위 12종을 만점까지 채운 사람뿐이라
