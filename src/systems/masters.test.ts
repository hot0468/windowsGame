...
import { MASTERS, MASTER_GIFT_RATIO, MASTER_MENTAL } from '../data/masters'
import { findItem } from '../data/items'
import { dueMasters, giftAmount, masterReached, receiveGift, reviveMasters } from './masters'
...
describe('선물 받기', () => {
  ...
  /**
   * ⚠️ **기록이 곧 사용권이다.** 이 검사가 깨지면 창을 다시 열어 같은 선물을 무한히 받는다.
   */
  it('두 번은 못 받는다', () => {
  ...
  /**
   * ⚠️ **문턱을 함수가 다시 본다.** "아직 안 받았나"만 보면 창을 열지 않고도 이 함수를
   * 부르는 통로가 게이트를 통째로 지나간다 — 규칙을 뒤집어 증명한다.
   */
  it('문턱을 안 넘었으면 창을 거치지 않고 불러도 아무 일이 없다', () => {
    const low = state()
