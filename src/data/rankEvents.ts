...
export interface RankEvent {
   ...
   * 이 스탯이.
   *
   * ⚠️ **2026-08-24에 다시 필수가 됐다.** 옵셔널이던 유일한 이유가 "생략하면 생활
   * 등급을 본다"였는데, 생활 등급이 삭제되면서(설계자 지시) 그 갈래가 사라졌다.
   * 되살리려면 판정 분기(`rankReached`)부터 다시 만들어야 한다.
   */
  key: GrowthStatKey
  ...
  target: string
  /**
   * **닿기 전에** 미리 적을 수 있는 한 줄. **(2026-08-24: 스탯창이 이 줄을 적던 자리는
   * 생활 등급과 함께 사라졌다 — 지금은 데이터로만 남아 있다.)** 예전 규칙: 생활 등급 이벤트
...
export const RANK_EVENTS: RankEvent[] = [
   ...
   * ── 말과 사람이 여는 자리 다섯 ───────────────────────────────
   *
   * ⚠️ **예전에는 `key`가 없는 "생활 등급 사다리"였다**(2026-08-24에 재배치).
   * 생활 등급(성장 스탯 평균)이 판을 이끄는 목표였고 이 다섯이 그 사다리의 칸이었는데,
   * 배경이 "20대의 딱 1년"으로 정해지면서 **목표 자리를 1년이 가져갔다** — 등급이
   * 그 자리를 대신할 이유가 사라져 통째로 걷어냈다(설계자 지시).
   *
   * 다섯은 **살리되 어울리는 스탯으로 옮겼다**: 모임은 사람을 두루 만난 값(친화력),
   * 칼럼·에세이는 쓰는 일(어휘력), 강연은 사람 앞에 서는 힘(매력), 인터뷰는 남들이
   * 나를 아는 정도(평판)다. **활동 4종과 대화방 4개는 그대로 산다** — 게이트만 바뀌었다.
   */
  {
    /* 친화력 B. 사람을 두루 만나 온 사람에게 오는 자리 — 매주 다른 사람이 와서
       자기가 아는 것을 푸는 모임이라, 넓게 아는 것 자체가 자격이다. */
    id: 'mentor-circle',
    key: 'sociability',
    rank: 'B',
    kind: 'thread',
  ...
  {
    /* 어휘력 B. 쓰는 일이라 어휘가 자격이다(활동도 어휘력을 4 올린다). */
    id: 'column-desk',
    key: 'vocabulary',
    rank: 'B',
    kind: 'thread',
  ...
  {
    /* 칼럼(어휘력 B)의 윗칸 — 같은 이야기가 책 한 권이 된다. */
    id: 'essay-press',
    key: 'vocabulary',
    rank: 'A',
    kind: 'thread',
  ...
  {
    /* 매력 S. 무대에서 삶 전체를 말하는 자리라 **사람 앞에 서는 힘**이 자격이다
       (활동도 매력·친화력·평판을 함께 올린다). */
    id: 'stage-hall',
    key: 'charm',
    rank: 'S',
    kind: 'thread',
  ...
  {
    /* 평판 S. 삶을 묻는 인터뷰라 **남들이 나를 아는 정도**가 자격이다.
       문구·아이콘은 `data/events.ts`가 갖는다(단발 이벤트 공통 규칙). */
    id: 'life-portrait',
    key: 'reputation',
    rank: 'S',
    kind: 'event',
