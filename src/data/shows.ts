 ...
 * ── 예매 ───────────────────────────────────────────────────────────────────
 *
 * ⚠️ **예매는 관람이 아니다**(설계자 지시). 표를 사면 그 자리에서 공연을 보는 게 아니라
 * **며칠 뒤 슬롯에 예약이 걸린다** — 실행되는 활동은 여전히 `concert` 하나이고,
 * 관람료·행동력·멘탈은 **공연 당일** 그 활동이 정산한다(예매 자체는 턴을 안 쓴다.
 * 은행·쇼핑 주문과 같은 부류다).
 *
 * ⚠️ **공연 기간(`period`)과는 여전히 무관하다.** 기간은 표시 전용이고(위 주석),
 * 게임 날짜를 잇는 것은 아래 상수 하나뿐이다 — 공연마다 날짜를 달면 "그 공연은 이미
 * 끝났다"는 정산 규칙이 새로 생긴다.
 */

/** 예매한 날부터 관람일까지의 최소 간격(일). */
export const BOOKING_LEAD_DAYS = 3

/**
 * 좌석 구역. **배열 순서가 곧 왼쪽부터의 배치**이고 가운데가 넓다(실제 공연장 판형).
 * ⚠️ 좌석은 게임 수치가 아니라 **판의 크기**다 — 늘리면 불이 들어온 자리를 찾기 어려워져
 * 난이도가 오른다. 그것이 여기 있는 이유다.
 */
...
/** 한 줄의 좌석 수. 좌석 번호는 이 값으로 줄과 구역을 가른다. */
export const SEAT_COLS = SEAT_BLOCKS.reduce((n, b) => n + b.cols, 0)
export const SEAT_COUNT = SEAT_ROWS * SEAT_COLS

/**
 * 좌석 하나에 불이 들어와 있는 시간(ms). 이 안에 못 누르면 불은 다음 자리로 옮겨 간다.
 * ⚠️ **이 값이 곧 예매의 난이도다.** 0.5초는 "보고 나서 누를 수 있는" 최소선이다.
 */
export const SEAT_LIT_MS = 500

/** 불이 꺼진 자리를 누를 수 있는 횟수. 넘기면 좌석이 다 나가고 예매가 실패한다. */
export const SEAT_MISS_LIMIT = 3
