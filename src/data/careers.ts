...
/**
 * 직업 레벨이 한 칸 오르는 데 필요한 **출근 횟수**(도감이 읽는 값).
 *
 * ⚠️ **레벨은 급여도 승진도 만들지 않는다 — 기록일 뿐이다.** 레벨이 돈을 건드리는 순간
 * 급여의 단일 출처(`Career.salary`)가 둘로 갈라지고, "정규직은 후반에 반드시 무너진다"는
 * 장치가 무너진다. 오래 다닌 사실을 도감에 남기는 것이 이 값이 하는 일 전부다.
 *
 * 한 급여 주기(15일)의 근무일이 10~11일이므로 5는 **반 주기 개근 = 한 칸**이다.
 */
...
export const CAREERS: Career[] = [
  {
    /**
     * ⚠️ **이 회사만 출근이 미니게임을 연다**(`data/callcenter.ts`의 `CALL_CENTER_CAREER_ID`).
     * 급여를 가장 낮게 잡은 것이 그 대가의 반대편이다 — 손을 쓰면 보너스로 메울 수 있고
     * 자동으로 넘기면 기본급만 받는다. **관계는 콜센터 → 공고 한 방향으로만 적는다**
     * (공고에 미니게임 플래그를 또 달면 한쪽만 고쳐도 아무 테스트가 안 터진다).
     */
    id: 'hanul-call',
    company: '한울텔레콤 고객센터',
    title: '고객상담원 (정규직)',
    ...
    salary: 1_500_000,
    paper: { vocabulary: 15 },
    person: { sociability: 15, charm: 10 },
