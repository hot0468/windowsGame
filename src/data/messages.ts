...
export const THREADS: Thread[] = [
  ...
  {
    /*
     * 다섯 번째 오픈채팅 — **취업 스터디**(2026-08-22 설계자 지시).
     *
     * ⚠️ **조건이 없다**(오픈채팅 규칙) — 문턱은 스탯이 아니라 참가비다. 취업은 이 게임의
     * 큰 축인데(벼룩장터 정규직) 여태 그 준비는 혼자 하는 `study` 하나뿐이었다.
     * ⚠️ **주간 예약이 없다** — 월~토가 이미 찼고(헬스장 목·미용실 토·러닝 수), 일요일
     * 하나를 태우기 전에 비주간 제안으로 되는지 먼저 본다는 규칙을 따른다(호텔 라운지와
     * 같은 판형: 선택지가 활동을 그 자리에서 실행한다).
     * ⚠️ **정기권을 팔지 않는다** — 서비스가 아니라 사람들 모임이라 "끊는다"가 성립하지
     * 않는다. 파는 것이 없는 방(러닝크루)과 파는 방(헬스장) 사이가 이 방의 자리다.
     */
    id: 'job-study-room',
    app: 'kakao',
    name: '취업 스터디 오픈채팅',
    ...
    open: true,
    offer: {
      question: '이번 주 스터디 나오실 분?',
      options: [
        {
          id: 'job-study-join',
          label: '나갈게요',
          desc: '스터디카페 8,000원 · 3시간 · 지식·어휘력·친화력이 함께 오릅니다',
          activityId: 'job-study',
        },
      ],
    },
  },
