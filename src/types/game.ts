...
export type WindowKind =
  ...
  | 'wish'
  /**
   * 스탯 마스터의 방문. **별똥별과 같은 부류다** — 스스로 열 수 없고, 등급이 문턱을 넘은
   * 밤에 저절로 뜨며, 턴을 쓰지 않는다. 어느 스승인지는 창 id가 나른다(`master-<스탯>`).
   */
  | 'master'
  /**
...
export interface GameState {
   ...
   * 선물을 **받은** 스탯 마스터의 id.
   *
   * ⚠️ **"찾아왔다"가 아니라 "받았다"의 기록이다**(별똥별과 같은 규칙) — 창을 닫기만 한
   * 사람은 기회를 잃지 않고 다음 밤에 다시 찾아온다. 규칙은 `systems/masters.ts`.
   *
