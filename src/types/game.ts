 ...
 * "카이로소프트처럼 완전한 게임오버는 없었으면"). **되살리지 말 것** — 자원이 바닥났다고
 * 판이 끝나면 그때부터 이 게임은 "안 죽기"가 목표가 된다.
 *
 * ⚠️ **결산일(`Settlement`)과 혼동하지 말 것.** 2026-08-24에 "1년"이라는 기한이
 * 생겼지만 그것은 **끝나는 날이 아니라 돌아보는 날**이다: 그날 이후로도 계속 살고,
 * 굳는 것은 직업·거주뿐이다. 게임오버는 여전히 없다.
 */
export type RecoveryKind = 'bankrupt' | 'burnout'

/**
 * 1년이 끝난 뒤의 상태 — **결산**(2026-08-24 설계자 설정: "20대의 딱 1년").
 *
 * ## ⚠️ 게임오버가 아니다
 * 결산일(`DEADLINE_DAY`)이 와도 판은 이어진다. 달라지는 것은 **진로를 더 못 바꾼다**는
 * 것뿐이다(설계자 지시: "회사에 들어가서 일을 하고 있으면 그 회사 일만 계속"). 그래서
 * 이 필드는 "끝났다"가 아니라 **"굳었다"**를 뜻한다 — 공부·운동·관계·알바는 그대로 된다.
 *
...
export interface Settlement {
  /** 결산한 날(= `DEADLINE_DAY`). 화면이 "몇 일차에 1년이 끝났나"를 적는다. */
  day: number
  /** 결산 시점에 다니던 회사. 없으면 무직으로 1년을 마쳤다는 뜻이다. */
  careerId?: string
  /** 결산 시점의 집. 이 뒤로 이사가 막힌다. */
  housingId?: string
}
...
export interface GameState {
  ...
  /**
   * **1년이 끝났는가**(`Settlement`). 없으면 아직 살고 있는 중이다.
   *
   * ⚠️ **게임오버 플래그가 아니다** — 이 값이 있어도 날짜는 그대로 흐르고 활동도 된다.
   * 막히는 것은 **진로를 바꾸는 것**(새 직장 지원·이사)뿐이다(`canRun`).
   */
  settled?: Settlement
  /**
