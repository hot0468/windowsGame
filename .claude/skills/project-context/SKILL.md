...
## ⏳ 진행 중 작업 (다른 컴퓨터에서 이어하기)
HUD를 **밝은 모던 시스템 디자인**으로 리스타일하는 중 중단됨. 디자이너 지시 5건:
1. 다크 판타지 UI 폐기 → 밝은 UI (직전 커밋 ca96b9c의 다크 방향은 기각됨. 뼈대만 유지)
2. 모던 시스템 디자인 (윈도우 11 위젯/SaaS 카드 느낌, 장식적 RPG 요소 제거)
3. 날짜 HUD 타이틀 영역 삭제 (내용만. 스탯창 헤더는 플레이어명이라 유지 가능)
4. 작업표시줄 시계 왼쪽 패널 버튼 → mdi-light 라인 아이콘 (설치됨, 304개)
5. 프로그램 아이콘(메신저·인터넷) → devicon 로고 참고 (설치됨: chrome/firefox/slack 등 1,042개)
- WIP 커밋의 src 변경분이 이 방향의 부분 진행분이다. 빌드·테스트(177) 통과 상태
- 검증: 대비 ≥4.5:1 실측, 패널 겹침 없음, CDP 실클릭 (기존 6회 선례 참조)
