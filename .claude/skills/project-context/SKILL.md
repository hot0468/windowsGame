...
- ⚠️ **HUD 시각 언어 = 밝은 모던 시스템 카드**(윈도우 11 위젯 / SaaS 대시보드). **다크 판타지 방향(근검정 + 샴페인 골드 + 금테 + 세리프 영문 부제 + ✳ 오너먼트)은 설계자에게 기각되어 폐기됐다 — 되살리지 말 것.**
  ...
  - ⚠️ **평판·도덕·예의범절의 상한은 100이다**(나머지 999). 단일 출처는 `systems/turn.ts`의 **`growthCap(key)`** — `clampStats`·게이지·**랭크**가 같은 함수를 본다. 어떤 엔딩 조건도 이 셋을 쓰지 않는다. **이 셋을 쓰는 엔딩을 추가하면 조건을 100 이하로 잡을 것**
  - ⚠️ **등급 뱃지(`.stat-rank`)는 자원 줄(평판·도덕)과 성장 그리드 전 칸에 붙는다. 행동력·멘탈·체력에는 안 붙는다** — 매 턴 오르내리는 **잔량**이지 쌓아 올린 것이 아니라 "등급"이 성립하지 않는다. **등급별 색은 없다**(액센트 하나 규칙) — 상위 두 등급(S·SS)만 같은 액센트를 **채워** 구분한다
  - ⚠️ **예의범절은 상한이 100인데도 자원 줄이 아니라 그리드에 있다** — 자원 줄 한 칸(약 46px)은 그리드 칸의 두 배이고 **스탯창은 세로 스크롤바가 뜨면 안 된다**(현재 패널 544px / 720px 화면 상한 557px). **자원 줄로 되돌리려면 먼저 다른 줄을 빼라.** `.stat-cell`의 `min-height: 22px`+`padding: 1px`과 `.stat-rank`의 `line-height: 1`도 같은 이유로 **키우면 안 된다**(11줄에 곱해진다)
...
- ⚠️ **활동을 실행하는 사이트 8종:** **미디북스**(`render: 'library'` → `reading`) · **시집이**(극장 예매, `'cinema'` → `movie`) · **아점**(`'publish'` → `writing`) · **알바몬**(`'jobs'` → **알바 4종** = 일용직) · **벼룩장터**(`'career'` → **정규직** 지원/면접/출근) · **슬로우캠퍼스**(`'campus'` → **강의 8종**, 아래) · **O넷**(`'cert'` → **자격시험**, 아래) · **트위터**(`'twitter'` → `sns`, 아래)
  ...
  - ⚠️ **O넷(`data/certs.ts`+`systems/certification.ts`+`CertSite`, `--qn-*` 보라 #6D28D9)은 알바몬·슬로우캠퍼스와 같은 구조다** — `Site.activityId`(`exam`)는 기본값이고 **응시료(`Cert.fee`)만 종목이 갖는다**. 활동 `exam`은 **`requiresPick`**이라 스케줄러·바로 가기에서 빠진다. 보라인 것은 벼룩장터 #2563EB·알바몬 #0369A1·은행 navy로 **파란 사이트가 이미 셋**이기 때문(color `Flashcard & Study Tool`, style `Accessible & Ethical`+`Data-Dense Dashboard`)
  - ⚠️ **합격은 응시 즉시가 아니라 발표일에, 그날의 스탯으로 확정된다**(설계자 지시). 무작위 없음 — 판정·사유의 단일 출처는 정규직과 같은 `shortfalls()`다. 상태는 `GameState.exams`(옵셔널, 사실만). 정산은 `afterTurn`의 **`advanceCertification`** 한 줄이고 **돈을 만지지 않으므로 `nightPayoutPending`에 원천을 더하지 않는다**. ⚠️ **새 알림 창구를 만들지 않았다**: 합격 자격증은 **`arrivals`**(택배 토스트)를, 합격·불합격 통지는 `examMessages()`가 **채널 `MAILBOX.id`**로 아웃룩을 그대로 탄다
  ...
  - ⚠️ **알바몬과 벼룩장터는 다른 것이다**(일용직 vs 지속 고용). 색 계열도 갈라 뒀다: 알바몬 하늘색 `--ab-*` #0369A1 / 벼룩장터 남색 `--fl-*` #2563EB(color `Classifieds / Buy-Sell`). 벼룩장터 판형은 style `Editorial Grid / Magazine` + `Data-Dense Dashboard` ⚠️ **알바몬만 활동이 여럿이다.** `Site.activityId`는 "아무것도 안 고른 상태의 기본값"(`work`)이고 실제 실행 활동은 고른 공고(`data/jobs.ts`의 `Job.activityId`)가 정한다. 공고 8개 / 활동 4종이라 **같은 직종 2개씩**이 같은 활동을 가리킨다(일당이 같아야 표시가 참이다). **공고에 급여를 다시 적지 않는다** — 카드의 일당은 `previewActivity`가 돌려준 money 행을 읽는다
  - ⚠️ **조건 미달 공고는 감추지 않고 비활성으로 보여 주고 사유를 글자로 적는다**("매력 12 이상 필요 — 현재 10"). 판정은 `canRun`이 하고 화면은 **사유만 파생**한다(두 번째 판정 규칙 금지) **활동은 `Site.activityId`로 가리키기만 한다** — 수치를 사이트에 다시 적으면 밸런스 테스트가 못 보는 두 번째 출처가 생긴다. id 유효성은 `sites.test.ts`가 순회로 지킨다
...
- **이사**: 수치 `data/housing.ts`(매물 5종), 규칙 `systems/housing.ts`, 화면 `apps/sites/RealtySite.tsx`(`--rt-*` 청록, `render: 'realty'`). 상태는 `GameState.housing`(옵셔널 = 시작 원룸). ⚠️ **은행과 같은 부류다 — 활동을 실행하지 않고 턴도 쓰지 않는다**(`activityId` 없음, `sites.test.ts`가 지킨다)
  - ⚠️ **보증금은 돌려받고 중개수수료만 사라진다.** 그래서 계약금은 `새 보증금 + 수수료 − 지금 보증금`이고, **매몰비용이 수수료뿐**인 것이 "매일 방을 옮기는 것"을 막는 유일한 장치다. 낸 금액은 `HousingState.deposit`에 **박아 둔다**(`TermDeposit.rate`와 같은 규칙) ⚠️ **싼 방일수록 보증금이 비싸다 — 방향을 뒤집지 말 것.** 돌려주는 돈으로 곡선을 반대로 잡으면 **1일차에 고시원으로 내려가는 것이 공짜 이득**이 되어 이사가 판단이 아니라 절차가 된다. 계약금 상한은 이 게임이 실제로 만드는 현금(알바 최적 플레이 정점 약 265만) **안에** 있어야 한다 — 넘으면 아무도 못 가는 매물이 된다
...
- **복권**: 수치 `data/lottery.ts`, 규칙 `systems/lottery.ts`, 화면은 **쇼핑 사이트 안의 판매대**(`ShopSite`의 `LotteryCounter`) — 계산대 옆에서 충동적으로 사는 것이라 별도 사이트로 만들지 않는다. 상태는 `GameState.lottery`(옵셔널)
  - ⚠️ **`Math.random` 대신 시드 PRNG(`ticketRoll(serial)`)를 쓴다.** 시드는 **구매 일련번호**라 장마다 새로 굴러가고(설계자 지시: "살 때마다 확률이 새로 나온다"), 그 번호가 세이브에 남아 **새로 고침해도 재굴림이 없다** — 없으면 새로 고침이 최적 전략(세이브 스커밍)이 된다 ⚠️ **기대값은 반드시 표 값보다 낮다**(현재 환급률 27.5%). `lottery.test.ts`가 **상금표에서 직접 계산해** 지킨다(은행 이율 부등식과 같은 장치) — 뒤집히면 복권이 수입원이 되고 파산 보증이 죽는다. 다만 **최하 등수는 자주 걸려야 한다**(1/10) — 아무것도 안 터지면 그냥 돈 지우는 버튼이다
...
- 미구현(별도 계획 필요): **평판 시스템**(트위터 화면은 있으나 게시·팔로워 변동은 없다 — 표시는 `reputation` 파생), **솔리테어**(`kind: 'solitaire'`만 예약됨), 엔딩 도감 UI, **랜덤 이벤트**(정의는 `data/events.ts`에 있고 발생 지점만 없다), 휴지통
