...
- ⚠️ **HUD 시각 언어 = 밝은 모던 시스템 카드**(윈도우 11 위젯 / SaaS 대시보드). **다크 판타지 방향(근검정 + 샴페인 골드 + 금테 + 세리프 영문 부제 + ✳ 오너먼트)은 설계자에게 기각되어 폐기됐다 — 되살리지 말 것.**
  ...
  - ⚠️ **예의범절은 상한이 100인데도 자원 줄이 아니라 그리드에 있다.** 자원 줄 한 칸은 게이지까지 약 46px이고 그리드 칸은 그 절반인데, **스탯창은 세로 스크롤바가 뜨면 안 된다**. 실측: 자원 줄에 두면 패널 589px → 세로 720px 화면의 상한(557px)을 넘겨 스크롤바가 생겼다. 지금은 544px이다. **자원 줄로 되돌리려면 먼저 다른 줄을 빼라.** 같은 이유로 `.stat-cell`의 세로 여백(`min-height: 22px` + `padding: 1px`)과 `.stat-rank`의 `line-height: 1`도 **실측으로 정한 값이다** — 키우면 11줄에 곱해져 그대로 스크롤바가 된다
