...
- 아이콘은 **lucide-react 컴포넌트를 데이터에서 직접 참조**한다. `Activity.icon`/`Ending.icon`/`OpenWindow.icon`/`Window` prop의 타입은 `IconComponent`(`src/types/game.ts`). 이모지 문자열 금지. 스탯별 아이콘·색은 `src/data/statMeta.ts`의 `STAT_META`가 단일 출처
...
- React 19.2 / Vite 8.2 / TypeScript 7.0 / Zustand 5.0 / Vitest 4.1 / lucide-react 1.28(아이콘)
...
- 데이터(수치): `src/data/` — activities(활동 5종, `onDesktop` 플래그), economy(물가 구간 6단계), endings(엔딩 6종), statMeta(스탯별 아이콘·색 + 표시 순서)
