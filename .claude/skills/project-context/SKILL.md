...
| 스탯 | 13종. ⚠️ **`stamina` = "행동력", `maxStamina` = "체력"으로 표시된다**(코드 키는 그대로). 소모 자원: `stamina`/`maxStamina`/`mental`(0~100)/`money`. 성장 스탯 **11종**: `knowledge`·`charm`·`sensitivity`·`reputation`·`morality`·`creativity`·`sociability`·`vocabulary`·`athletics`·`gaming`·`manners`(예의범절). 상한은 `growthCap(key)`가 정한다 — 평판·도덕·**예의범절**만 100, 나머지 999 |
...
| 활동 선택 | ⚠️ **바탕화면에 활동 아이콘은 없다**(`onDesktop` 전부 false). 실행 통로는 **넷**: ①카톡 대화창 [만나러 가기](=`social`) ②스케줄러 예약 ③브라우저 사이트의 확정 버튼 ④바탕화면 바로 가기. 활동 **29종** 정의는 `data/activities.ts` |
...
- ⚠️ **네이놈 포털은 "모던"이다. 되살리지 말 것:** 검색창의 3px 초록 테두리, 카드 제목 밑 2px 초록 밑줄, 즐겨찾기 줄의 위아래 가로줄, 목록 항목마다 그은 구분선. 현재는 **1.5px 초록 링 + 브랜드 글자("네")를 품은 알약 검색창**, **원형 아이콘 판 퀵메뉴**, 큰 모서리(`--r-xl`) 카드, **리드 기사 1건 + 출처·시각 메타 줄**이다. 뉴스 시각은 인덱스에서 뽑는다(`Date`를 쓰면 결정성이 깨진다). 로고 서체는 `--font-logo`(SB 어그로). 사이트 아이콘은 `fluent-color`. ⚠️ **검색 실행 글리프만 단색(`mdi:magnify` + 브랜드 초록)**이다(정체성=컬러 / 컨트롤 글리프=단색, 단색이라야 CSS로 초록을 입힌다). **브랜드 초록은 로고·태그·포커스에만.**
...
- ⚠️ **활동을 실행하는 사이트 8종**(하이마루·은행·부동산은 실행하지 않는다)**:** **미디북스**(`render: 'library'` → `reading`) · **시집이**(극장 예매, `'cinema'` → `movie`) · **아점**(`'publish'` → `writing`) · **알바몬**(`'jobs'` → **알바 4종** = 일용직) · **벼룩장터**(`'career'` → **정규직** 지원/면접/출근) · **슬로우캠퍼스**(`'campus'` → **강의 8종**, 아래) · **O넷**(`'cert'` → **자격시험**, 아래) · **트위터**(`'twitter'` → `sns`, 아래)
  - ⚠️ **트위터(`data/tweets.ts`+`TwitterSite`, `--tw-*` 검정)는 고를 것이 없는 유일한 활동 사이트다** — `Site.activityId`(`sns`)가 "기본값"이 아니라 실행 활동 **전부**다(알바몬·캠퍼스·O넷 규칙과 갈린다). **탭(추천/팔로잉)·검색·트렌드는 목록을 거를 뿐 상태를 읽기만 한다.** 트렌드는 `news.ts`의 `TRENDING_TERMS` 재사용이고 팔로워 수는 `reputation` 파생이라 **새 상태가 없다**(평판 시스템은 여전히 미구현). 레퍼런스(실제 X)에서 **구독 박스·팔로우 추천·갈 데 없는 네비 항목을 뺐고 반응 숫자는 버튼이 아니다** — 장식 금지 규칙이다. 작성창 자리에는 `ActivityCommit`을 그대로 놓는다(확정 버튼이 둘로 보이면 안 된다).
...
### 쇼핑 · 택배 · 폴더
- ⚠️ **물건을 파는 사이트가 둘이다: 쇼핑(생활용품) / 하이마루(전자기기, `render: 'tech'`, `--tc-*` 주황 #C2410C).** 진열 축은 **`ShopItem.store`**(`'shop'|'tech'`, 생략 = 쇼핑)이고 목록은 **`buyableFor(store)`가 파생**한다 — `SHOP_ITEMS`는 여전히 물건 전체의 단일 출처다(인벤토리·탐색기가 이름을 찾는 곳). **사이트별 배열을 따로 적지 말 것.** 하이마루도 은행·부동산처럼 **`activityId`가 없고 턴을 쓰지 않으며** `systems/delivery.ts`를 그대로 탄다(새 배송 경로 금지). ⚠️ **잠금 사유의 가게 이름은 `storeNameOf(itemId)`가 정한다** — 화면에 "쇼핑에서 구입"을 다시 적으면 전자기기를 옮긴 순간 거짓이 된다. ⚠️ **살 수 있는 물건의 아이콘은 서로 달라야 한다**(`delivery.test.ts`가 순회로 지킨다 — 자격증 6종의 `certificate-24` 공유는 `buyable: false`라 예외)
...
- 데이터(수치): `src/data/` — **videos**(`VIDEOS`·`SHORTS`·`CHANNELS` — 썸네일은 그라데이션+글자), **items**(`SHOP_ITEMS`·`fakeSize`), **events**(`EVENTS`), activities(활동 **29종** + `ACTIVITY_CATEGORIES` + `WORK_ACTIVITIES`(`burnoutKey === 'work'`에서 파생 — 목록을 따로 적으면 한쪽만 고친다) + `activitiesOf`/`activitiesUnlockedBy`, `onDesktop`은 현재 전부 false), **jobs**(`JOBS`·`jobsOf`·`findJob` — 알바몬 공고 8개. **수치가 없다**: `activityId`로 가리키기만 한다), **bank**(이율·한도·만기 상수. ⚠️ `LOAN_RATE > DEPOSIT_RATE > SAVINGS_RATE`), **certs**(자격 종목 4종 + `certForItem`. ⚠️ **응시료(`fee`)·합격 기준·발표 소요일을 여기가 갖는다**), **courses**(강의 8종 + `CERTIFICATE_SESSIONS` + `courseForCertificate`. ⚠️ **수강료만 여기가 갖는다**), **careers**(`CAREERS`·`findCareer`·**`careerRank`**·**`careersRequiring`**·채용 일정/급여 주기/결근 상수 — 정규직 공고 5개. ⚠️ 알바와 반대로 **급여를 여기가 갖는다**), **startMenu**(`START_MENU_ITEMS`), **messages**(`CHAT_APPS`·`THREADS`·`MAILBOX`·`MESSAGE_SCHEDULE`), **banners**(`BANNERS`), desktopItems(`DESKTOP_ITEMS` — 바탕화면 아이콘 단일 출처 + `desktopEntries`), **desktopIcons**(`DEFAULT_ICON_CELLS`·`DESKTOP_ICON_ORDER` — 기본 격자 배치), **sites**(`SITES`·`BOOKMARK_SITES`·`HOME_SITE_ID`·`findSite`·`resolveUrl`), **media**(`BOOKS`·`FILMS`·`WRITING_PROMPTS`·`MAIN_FILM_ID`·`findShowtime`. ⚠️ `Film.section`이 `now`/`soon`/`arte`를 가르고 **`soon`만 회차가 없다**. 포스터도 그라데이션이다), **news**(`NEWS_POOL`·`NEWS_VISIBLE_COUNT`·`TRENDING_TERMS`), **tweets**(`TWEET_ACCOUNTS`·`TWEETS`·`tweetAge`·`followersFrom`·`countLabel`. ⚠️ **트렌드를 여기에 다시 적지 않는다** — `news.ts`의 `TRENDING_TERMS`를 쓴다), **autoAdvance**(자동 진행 수치), layers(`LAYERS`), shell(`SHELL` + **`DESKTOP_GRID`** + **`MOBILE_SHELL`**), calendar(날짜 환산·날짜칸 배치), economy(물가 구간 6단계), housing(매물 5종)·lottery(상금표·표 값), endings(**11종** — `ACHIEVEMENT_ENDINGS` 4 + **`CAREER_ENDINGS` 5**(`careerEnding(careerId)`) + `FAILURE_ENDINGS` 2), statMeta(`icon`(다색)·`hudIcon`(단색) + 표시 순서), icons(`UI_ICONS`/`HUD_ICONS`/`BROWSER_ICONS`)
- 훅: `src/hooks/` — **useShell**(셸 판정 + resize 구독)
- 아이콘: `src/icons/` — AppIcon · bootstrap · generated.ts(자동 생성, 수정 금지). 생성기는 `scripts/build-icon-subset.mjs`
- 로직(순수함수): `src/systems/` — turn(활동 실행·슬롯 전환·취침 정산·게임오버 + **`nightPayoutPending`**(급여 + 정기예금 만기 + **복권 당첨금** 셋을 본다)·**`settleGameOver`**), economy(**`getLivingCost(state)`**·`livingCostForDay`·`tierCostFor`·알바비), burnout(연속 페널티 + **`burnoutKeyOf`** — 이력에 넣는 키와 세는 키를 한 함수로 묶는다. ⚠️ 키가 갈라지면 페널티가 조용히 0이 된다), ending(티어 판정 + `getFailureEnding(reason, state)`·**`epitaphCareerId`**), **rank**(스탯 등급 F~SS — `rankOf`·`toNextRank`·`rankOfRatio`. ⚠️ 상한은 `growthCap`에 물어본다), **news**, **browserHistory**, **messages**, **search**, **schedule**, **employment**(정규직 + **`recordPeakCareer`**. `turn.ts`를 부르지만 반대는 없다), **desktopGrid**, **shell**(`shellForWidth`·`resolveShell`), **shortcuts**(`shortcutIdOf`·`firstFreeCell`·`placeShortcuts`), **contextMenu**(`clampMenuPosition`), **autoAdvance**(`STOP_RULES`·`findStop`·진행 기록), **bank**(예금·대출 + **`bankNightCredit`**·**`advanceBank`**. `turn.ts`를 부르지만 반대는 없다), **housing**(이사 — `turn.ts`를 부르지만 반대는 없다), **lottery**(시드 PRNG `ticketRoll` · **`advanceLottery`**), **courses**(수강·수료증 — `takeCourse`·`blockReason`·`sessionsOf`), **certification**(응시·발표 — `takeExam`·`advanceCertification`·`examMessages`. 둘 다 `turn.ts`를 부르지만 반대는 없다), **delivery**(⚠️ `owns`/`inventoryOf`는 **turn.ts로 옮겨졌고 여기서 재수출**한다 — `canRun`이 보유를 봐야 해서 두면 순환이 된다). 각각 `.test.ts` 동반 + `balance.verify.test.ts` + `data/activities.test.ts`. 총 **624개** 테스트
- 상태: `src/store/` — **shortcutStore**(바로 가기 목록·영구), gameStore(세이브+loggedIn, **persist version 2**), metaStore(도감·영구), windowStore(창 목록 + 최소화/최대화·휘발), desktopPanelStore(패널 z·휘발), **desktopIconStore**(아이콘 격자 위치·영구, 옮긴 것만), **browserStore**(즐겨찾기 + 개발자 모드·영구), **toastStore**(휘발), **shellStore**(셸 override·영구)
- UI: `src/components/`(**PanelOrnament** / **ContextMenu**), `window/`(Window·WindowManager·**appForWindow** — kind 분기 단일 출처), `desktop/`(Desktop·HudPanel·StatPanel·CalendarPanel·Taskbar·StartMenu·ToastHost), `lockscreen/`, **`mobile/`**(MobileShell·MobileAppView·MobileStatusBar·MobileNavBar·MobileStatSheet — 모바일 셸), `apps/`(ExeApp·StubApp·EndingModal·BrowserApp·ChatApp·MailApp·SystemApps·SchedulerApp·ExplorerApp·**AutoLogApp**·**ActivityConfirm**), `apps/sites/`(NeverPortal·ShopSite·ConstructionSite·**ActivityCommit**·LibrarySite(미디북스)·CinemaSite(시집이)·PublishSite(아점)·AlbamonSite·FleaSite·**BankSite**·RealtySite·**CampusSite**(슬로우캠퍼스)·**CertSite**(O넷)·**TwitterSite**(트위터)), `apps/activityPreview.ts`(`previewActivity` + **`previewWarnings`** + **`blockReasons`** — 확정 화면 셋이 공유)
- 설정: `vite.config.ts`, `tsconfig.json`(+`.app`/`.node`), 전역 CSS `src/index.css`(**디자인 토큰 `:root` 단일 출처**)
- 미구현(별도 계획 필요): **평판 시스템**(트위터 화면은 있으나 게시·팔로워 변동은 없다 — 표시는 `reputation` 파생), **솔리테어**(`kind: 'solitaire'`만 예약됨), 엔딩 도감 UI, **랜덤 이벤트**(정의는 `data/events.ts`에 있고 발생 지점만 없다), 휴지통, **사이트·앱 내부 모바일 반응형**(아웃룩 3단·스케줄러 등이 375px에서 `.mo-app-body` 안 가로 스크롤로 흐른다 — 셸은 안 밀린다), **잠금화면 모바일 대응**
