...
- ⚠️ **예약 취소에는 확인을 받는다**(설계자 지시, ux `confirmation-dialogs`). 예약된 슬롯을
  누르면 바로 지우지 않고 alertdialog가 뜬다. 기본 초점은 **덜 위험한 [그대로 두기]**에 둔다.
  `window.confirm`은 쓰지 않는다 — 가짜 OS 위에 진짜 브라우저 대화상자가 뜨면 컨셉이 깨진다.
