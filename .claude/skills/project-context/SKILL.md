...
- ⚠️ **활동 바로 가기**(2026-08-05 신설, 설계자 지시). **사이트 확정 버튼을 오른쪽 클릭 → [바탕화면에 등록] → 아이콘이 생기고, 더블클릭하면 확인창을 거쳐 그 활동이 바로 실행된다.** 실제 윈도우의 "바로 가기 만들기"이고, 바탕화면을 플레이어가 실제로 쓰는 활동의 빠른 실행 줄로 만든다.
  - **화면이 순회하는 목록은 하나다:** `data/desktopItems.ts`의 **`desktopEntries(shortcutActivityIds)`** = 내장 항목 + 바로 가기. 타입은 판별 합집합 `DesktopEntry`(`shortcut: false`면 `item`, `true`면 `activityId`). 격자·드래그·저장이 둘에게 완전히 같아야 해서 목록을 나누지 않았고, **더블클릭했을 때 하는 일만** 갈린다(창 열기 / 확인창).
  - **id는 `shortcut:<활동id>`**(`systems/shortcuts.ts`의 `shortcutIdOf`). 활동 하나당 하나뿐이라 중복 등록 판정이 그냥 되고, 일련번호와 달리 **새로고침해도 옮겨 둔 칸과 어긋나지 않는다**. 콜론이 내장 id(전부 평범한 낱말)와의 충돌을 막는다 — `shortcuts.test.ts`가 순회로 지킨다.
  - **목록은 `store/shortcutStore.ts`**(persist, 키 `windows-game-shortcuts`, `activityIds`만). ⚠️ **`gameStore`에 넣지 않는다** — `desktopIconStore`·`browserStore`와 같은 판단이다(바탕화면에 뭘 둘지는 판이 아니라 사람의 것). **새 판을 시작해도 남는다.** 안전한 근거: 바로 가기는 활동을 가리키기만 하고, 더블클릭할 때마다 그 판의 `canRun`을 다시 묻는다.
  - ⚠️ **위치는 `desktopIconStore`가 그대로 담는다**(id 하나로 두 스토어가 이어진다). 위치 규칙을 두 벌로 만들면 "바로 가기만 격자에 안 붙는" 버그가 생긴다. 배치는 **2단계**다: `resolveLayout`으로 내장 아이콘을 먼저 다 놓고, `systems/shortcuts.ts`의 **`placeShortcuts`**가 그 위에 얹는다(옮긴 칸이 있으면 그 칸, 없으면 **`firstFreeCell`** = **열 우선** 첫 빈 칸). 섞어 돌리면 바로 가기 하나가 내장 아이콘의 기본 칸을 뺏어 **기본 배치가 통째로 밀린다**. 실측 확인: 바로 가기를 만들어도 기존 7개는 **1px도 안 움직인다**
  - **바로 가기 화살표는 CSS 도형**(`.desktop-icon-link`, 흰 판 + `::before` 화살대 + `::after` 화살촉). 캡션 버튼·브라우저 글리프와 같은 이유다. 뜻이 있으므로 라벨에 스크린 리더용 "(바로 가기)"를 덧댄다
  - **확인창은 `components/apps/ActivityConfirm.tsx`**(`role="alertdialog"`, `LAYERS.DIALOG`). ⚠️ **`window.confirm` 금지**(스케줄러 취소 확인창과 같은 이유 + 크롬 대화상자엔 숫자를 담을 자리가 없다). 기본 포커스는 덜 위험한 [취소]. **`canRun`이 false면 실행 버튼을 아예 안 그리고 사유를 적는다** — 지름길이 정상 경로의 제약(행동력·소지금·`requires`·`requiresItem`·게임오버)을 하나도 건너뛰지 않는다
  - ⚠️ **경고 문구의 단일 출처가 생겼다: `apps/activityPreview.ts`의 `previewWarnings`**(번아웃·조건 미달·오후 생활비)와 **`blockReasons`**(왜 못 하는가). 활동을 확정하는 화면이 셋이 되어(활동 창·사이트 확정 패널·이 확인창) 각자 적으면 한 곳만 빠뜨린 화면이 반드시 생긴다. 판정은 여전히 `canRun`/`getBurnoutPenalty`/`getLivingCost`가 하고 여기서는 **문장으로 옮기기만** 한다
  - 삭제는 **바로 가기 아이콘 우클릭 → [바로 가기 삭제]**뿐이다. 내장 아이콘 메뉴에는 [열기]밖에 없다(실제 윈도우도 시스템 아이콘은 못 지운다). 삭제에 재확인을 받지 않는 이유: 되돌리는 비용이 우클릭 한 번이고, 옮겨 둔 칸까지 남아 같은 자리로 돌아온다(예약 취소와 다르다)
  - 등록은 브라우저 창 뒤에서 일어나 **눈에 안 보인다** → 확정 패널에 `.ac-added` 한 줄로 알린다(ux `success-feedback`). 이미 있으면 조용히 하나 더 만들지 않고 **"이미 바탕화면에 있습니다"** 비활성 항목으로 말해 준다
- ⚠️ **오른쪽 클릭 메뉴는 공용 부품 `src/components/ContextMenu.tsx`다**(2026-08-05 신설. 앞으로 휴지통·폴더·바탕화면 빈 자리도 이걸 쓴다). 인라인으로 만들면 열기·닫기·바깥 클릭·Esc·키보드 이동·화면 밖 클램프 **여섯 가지를 매번 다시** 만들어야 하고, 두 번째 자리에서 하나를 빠뜨린다.
  - ⚠️ **`document.body`로 포탈한다.** 사이트 확정 버튼은 브라우저 창 안에 있고 `.win-body`는 `overflow: auto`라 그 안에 그리면 **메뉴가 잘린다**. 창이 자기 z로 쌓임 문맥을 만들어 z를 올려도 작업 표시줄을 못 넘는다 — 포탈이 둘을 한 번에 푼다
  - 바깥 클릭은 **투명 판 한 장**(`.ctxmenu-scrim`)이 받는다(브라우저 ⋮ 메뉴와 같은 방식, 전역 리스너 정리 코드 없음). 화면 밖 클램프는 순수 함수 **`systems/contextMenu.ts`의 `clampMenuPosition`**이 하고 **넘칠 때 잘라 붙이지 않고 반대쪽으로 뒤집는다**(실제 윈도우와 같다. 실측: x=1223에서 열면 x=1043으로 뒤집힌다)
  - ⚠️ **고를 수 있는 항목이 하나도 없으면 판 자체에 포커스를 준다**(`tabIndex={-1}`, CDP 실측으로 잡은 버그). 비활성 버튼은 포커스를 못 받아, 안내 한 줄짜리 메뉴에서 **Esc가 메뉴에 닿지 않았다**
  - 열 때의 포커스를 기억했다가 닫을 때 **돌려준다**(ux `focus-management`). 아이콘은 우클릭 시 스스로 포커스를 잡는다(실제 윈도우도 우클릭한 아이콘이 선택된다)
  - ⚠️ **비활성 버튼은 마우스 이벤트를 발사하지 않는다.** 확정 버튼은 못 고른 상태에서 `disabled`이므로 우클릭은 **감싼 상자(`.ac-commit`)**가 받고, `.ac-btn:disabled { pointer-events: none }`이 판정을 그리로 흘려보낸다 — "지금은 못 하지만 자주 할 것 같다"가 바로 가기를 만들 이유이므로 그때 막히면 안 된다
...
- ⚠️ **간격 척도에 `--sp-7`은 없다**(4/8 리듬: 1·2·3·4·5·6·8 = 4/8/12/16/20/24/32). CSS는 **정의되지 않은 var를 만나면 그 선언 전체를 무효로 만든다** — `padding: var(--sp-7) var(--sp-6) 0`이 통째로 사라져 구역이 화면 왼쪽에 달라붙었다(시집이·아점에서 실제로 터졌다). 없는 토큰을 쓰면 빌드도 테스트도 통과하고 **화면에서만** 조용히 깨진다
...
- ⚠️ **브라우저는 사이트 컨테이너다** (2026-08-03 신설). `src/data/sites.ts`의 `SITES`가 사이트 단일 출처이고(id·가짜 URL·제목·아이콘·`render`·안내 문구·`bookmark` 플래그·`activityId`), `BrowserApp`은 **`site.render`로만 분기한다**(`'portal' | 'construction' | 'shop' | 'library' | 'cinema' | 'publish' | 'tube' | 'jobs' | 'career'`). 사이트 id로 분기하는 순간 "데이터 한 줄 + 컴포넌트 하나"로 사이트를 늘리는 구조가 무너진다. 새 사이트 추가 = SITES에 항목 하나 + (새 종류라면) `render` 값 하나와 컴포넌트 하나
  ...
  - ⚠️ **활동을 실행하는 사이트 5종** (2026-08-04 신설, 2026-08-05 알바몬·벼룩장터 추가). **미디북스**(밀리의 서재 패러디, `render: 'library'` → `reading`) · **시집이**(CGV 패러디 = **극장 예매**, `'cinema'` → `movie`) · **아점**(포스타입 패러디, `'publish'` → `writing`) · **알바몬**(`'jobs'` → **알바 4종** = 일용직) · **벼룩장터**(생활정보지 패러디, `'career'` → **정규직** 지원/면접/출근). 이것이 브라우저를 **활동 실행의 세 번째 통로**로 만든다.
    ...
    - ⚠️ **네 사이트(너튜브·시집이·아점·미디북스)는 2026-08-05에 전부 실제 서비스 홈 레퍼런스로 갈아엎었다.** 각자 자기 CSS 파일 안에 토큰을 가둔다: 너튜브 `--tb-*` / 시집이 `--cn-*` / 아점 `--pb-*` / 미디북스 `--lb-*`. 전역 팔레트를 사이트 수만큼 부풀리면 색의 출처를 아무도 못 찾는다. 간격·타입·모서리만 전역 토큰을 쓴다
    - 미디북스 = 흰 배경 + 파랑 액센트. 구조: 헤더 → 카테고리 탭 → 배너 캐러셀 → 퀵메뉴 원형 → 지금 많이 읽는 작품(별점 순 9권, 3열) → 오늘의 발견(표지 6열) → 이벤트 → 새로 나온 작품 → 확정 패널 → 푸터. `Book`에 `cover`(그라데이션)·`category`·`rating`·`ratings`·`badge`가 생겼고 책은 6권 → **20권**이다
    - ⚠️ **아점도 2026-08-05에 레퍼런스(실제 창작 플랫폼 홈)로 갈아엎었다.** 크림 배경 + 자간 벌린 영문 소제목, 토큰은 `.pub` 안에 `--pb-*`. 구조: 프로모션 띠 → 헤더 → 히어로 카피(아래로 갈수록 흐려지는 세 줄) → 대표작 캐러셀 → **KEYWORD 격자** → 요일별 연재 → 작가 소개 → 배너 → 확정 패널. **키워드 격자가 곧 글감 고르기다** — 레퍼런스의 판을 장식으로 베끼는 대신 게임 기능을 그 자리에 앉혔다. 그래서 `WritingPrompt`에 짧은 `keyword`가 생겼다(문장형 `theme`은 격자 칸을 무너뜨린다)
    - ⚠️ **`auto-fit` 격자의 열 수는 `minmax`의 최소 폭이 정한다.** 판이 1080px일 때 3열을 원하면 최소 폭이 1080/4보다 커야 한다 — 240px로 두면 4열이 되어 작가 6명이 "4 + 2"로 깨진다. 카드 폭에 상한을 거는 것으로는 못 막고, **판의 폭을 묶은 뒤 최소 폭을 계산**해야 한다
...
- ⚠️ **알바는 4종이다**(2026-08-05, 알바몬과 함께 신설). `work`(편의점, **조건 없는 유일한 알바**) ·
  `work-cafe`(카페, 매력 12 — 돈 대신 매력과 낮은 멘탈 소모를 산다) · `work-logistics`(물류센터, 운동 25) ·
  `work-tutor`(과외, 지식 60 — 행동력당 수입 최고). 설계 문서의 **"알바비는 물가보다 느리게 오른다 →
