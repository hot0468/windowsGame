...
| 활동 선택 | ⚠️ **바탕화면에 활동 아이콘은 없다**(`onDesktop` 전부 false). 활동을 실행하는 통로는 **셋**이다: ①카톡 대화창의 [만나러 가기](= `social`) ②**스케줄러 예약**(그 슬롯이 오면 자동 실행) ③**브라우저 사이트의 확정 버튼**(2026-08-04 신설 — 미디북스=`reading`·시집이=`movie`·아점=`writing`·**알바몬=알바 4종**). 활동 **18종** 정의는 `data/activities.ts`에 그대로 있다 |
